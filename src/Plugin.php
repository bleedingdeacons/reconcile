...
class Plugin
{
     ...
     * Registers all Reconcile services (lookups, importers, exporters,
     * and AJAX handlers) into Unity's DI container before wiring hooks.
     */
    ...
    /**
     * Register all Reconcile service definitions into the container.
     */
    private static function registerServices(ContainerInterface $container): void
    {
        // --- Lookups ---
        $container->register(GroupLookup::class, function (ContainerInterface $c) {
            return new GroupLookup(
                $c->get(GroupRepository::class)
            );
        });

        $container->register(PositionLookup::class, function (ContainerInterface $c) {
        ...
        // --- Group services ---
        $container->register(GroupImporter::class, function (ContainerInterface $c) {
        ...
        $container->register(GroupExporter::class, function (ContainerInterface $c) {
            return new GroupExporter(
                $c->get(GroupRepository::class)
            );
        });
        ...
        $container->register(MemberExporter::class, function (ContainerInterface $c) {
            return new MemberExporter(
                $c->get(MemberRepository::class),
                $c->get(GroupRepository::class),
                $c->get(PositionRepository::class)
            );
        ...
        // --- Position services ---
        $container->register(PositionImporter::class, function (ContainerInterface $c) {
            return new PositionImporter(
                $c->get(PositionRepository::class),
                $c->get(PositionFactory::class)
            );
        });

        $container->register(PositionExporter::class, function (ContainerInterface $c) {
            return new PositionExporter(
                $c->get(PositionRepository::class)
            );
        });

        // --- AJAX Handlers ---
        $container->register(GroupImportHandler::class, function (ContainerInterface $c) {
            return new GroupImportHandler(
                $c->get(GroupImporter::class)
            );
        });

        $container->register(GroupExportHandler::class, function (ContainerInterface $c) {
            return new GroupExportHandler(
                $c->get(GroupExporter::class)
            );
        });

        $container->register(MemberImportHandler::class, function (ContainerInterface $c) {
            return new MemberImportHandler(
                $c->get(MemberImporter::class)
            );
        });

        $container->register(MemberExportHandler::class, function (ContainerInterface $c) {
            return new MemberExportHandler(
                $c->get(MemberExporter::class)
            );
        });

        $container->register(PositionImportHandler::class, function (ContainerInterface $c) {
            return new PositionImportHandler(
                $c->get(PositionImporter::class)
            );
        });

        $container->register(PositionExportHandler::class, function (ContainerInterface $c) {
            return new PositionExportHandler(
                $c->get(PositionExporter::class)
            );
        });
    }

    /**
     * Retrieve all AJAX handlers from the container and register their hooks.
     */
