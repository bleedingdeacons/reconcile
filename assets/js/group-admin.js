(function ($) {
    ...
    $(function () {
        var $form     = $('#reconcile-group-import-form');
        var $submit   = $('#reconcile-group-submit');
        var $spinner  = $('#reconcile-group-spinner');
        var $results  = $('#reconcile-group-results');

        $form.on('submit', function (e) {
            e.preventDefault();

            var fileInput = document.getElementById('reconcile-group-file');
            ...
            formData.append('action', 'reconcile_group_import');
            formData.append('reconcile_group_nonce', $form.find('[name="reconcile_group_nonce"]').val());
            formData.append('import_file', fileInput.files[0]);

            if ($('#reconcile-group-dry-run').is(':checked')) {
            ...
            $.ajax({
                url: reconcileGroupAdmin.ajaxUrl,
                type: 'POST',
                data: formData,
                processData: false,
                contentType: false,
        ...
        function renderResults(data, isSuccess) {
            var isDryRun = $('#reconcile-group-dry-run').is(':checked');
            var html = '';
            ...
            if (data.skipped_rows && data.skipped_rows.length) {
                ...
                for (var k = 0; k < data.skipped_rows.length; k++) {
                    ...
                    html += '</tr>';
                    if (hasDetails) {
                        html += '<tr class="reconcile-details-row" id="reconcile-group-details-' + k + '" style="display:none;">';
                        html += '<td></td>';
        ...
        $results.on('click', '.reconcile-toggle-details', function (e) {
            ...
            var $detailsRow = $('#reconcile-group-details-' + idx);
