...
class PluginBuilder
{
    ...
    private string $pluginName = 'reconcile';
    ...
    {
        $mainFile = $this->pluginDir . DIRECTORY_SEPARATOR . 'reconcile.php';
        if (file_exists($mainFile)) {
            $content = file_get_contents($mainFile);
            if (preg_match('/Version:\s*([0-9.]+\s*\w*)/', $content, $matches)) {
                return trim($matches[1]);
            }
            if (preg_match("/define\s*\(\s*'RECONCILE_VERSION'\s*,\s*'([^']+)'/", $content, $matches)) {
                return $matches[1];
            }
        }
        return '0.0.1';
...
Reconcile WordPress Plugin Build Script
========================================
Platform: {$platform}
...
Note:
  Reconcile is a companion plugin to Unity. Make sure Unity is installed
  and activated before activating Reconcile.
